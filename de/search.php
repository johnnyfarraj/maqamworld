    ...
    foreach ($dir as $file){
        if (!$file->isDot() && endsWith($file, ".php")) {
            ...
            if (strpos($content, strtolower($string)) !== false) {
                ...
                array_push($files, array("title" => $title, "link" => str_replace('./', 'http://maqamworld.com/de/', $file->getPathname()), "appereances" => $appeardTotal));
                $cont++;
            }
        }
...
if(isset($_GET['q']) && strip_tags($_GET['q']) != "" && strlen(strip_tags($_GET['q'])) > 3) {
    ...
    $array = array('status' => "error", 'message' => "Bitte geben Sie das Suchwort ein (mindestens vier Zeichen).");
}
