<?php defined('_DEFVAR') or exit('Restricted Access'); ?>
    <footer>
    <div class="container">
        <div class="row">
            <div class="col-sm-4"> <img src="/img/logo-bottom.png" height="30" class="footer-logo">
                <p>&copy; 2001-2018 MaqamWorld
                    <br>Alle Rechte vorbehalten.
                    <br>Konzept, inhalt und design: <a href="https://www.amazon.com/Johnny-Farraj/e/B07SD841D2" target="_blank">Johnny Farraj</a>
                    <br>Programmierung: <a href="http://filiptodorov.com" target="_blank">Filip Todorov</a>
                    <br>Deutsche Übersetzung: Ines Körver
                </p>
                <!--
                <ul class="socials">
                    <li><a href="#"><i class="fa fa-fw fa-facebook"></i></a></li>
                    <li><a href="#"><i class="fa fa-fw fa-twitter"></i></a></li>
                    <li><a href="#"><i class="fa fa-fw fa-instagram"></i></a></li>
                    <li><a href="#"><i class="fa fa-fw fa-youtube"></i></a></li>
                </ul>
                -->
            </div>
            <div class="col-sm-3">
                <h3>Kontakt</h3>
                <ul class="menu">
                    <!--
                    <li><a href="#"><i class="fa fa-fw fa-phone"></i>Rufen Sie uns an</a></li>
                    <li><a href="#"><i class="fa fa-fw fa-map-marker"></i>Besuchen Sie uns</a></li>
                    -->
                    <li><a href="/de/contact.php"><i class="fa fa-fw fa-envelope"></i>Schreiben Sie uns</a></li>
                </ul>
            </div>
            <div class="col-sm-5 text-right">
                <a href="https://www.arabculturefund.org/indexarabic.php" target="_blank"><img src="/img/afac_en.png" class="afac"></a>
            </div>
        </div>
    </div>
</footer>