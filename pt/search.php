...
function searchDirectoryIterator($path, $string){
    ...
    foreach ($dir as $file){
        if (!$file->isDot() && endsWith($file, ".php")) {
            ...
            $content2 = curl_get_contents(str_replace('./', 'http://maqamworld.com/pt/', $file->getPathname()));
            ...
            if (strpos($content, strtolower($string)) !== false) {
                ...
                array_push($files, array("title" => $title, "link" => str_replace('./', 'http://maqamworld.com/pt/', $file->getPathname()), "appearances" => $appeardTotal));
                $cont++;
            }
        }
